...
const PostCard = ({ reloadPosts, avatar }) => {
    ...
    useEffect(() => {
        const fetchFavoritePosts = async () => {
            try {
                const response = await axios.get(`${Url}favorites/get?page=0&pageSize=10&sortName=createAt&sortType=DESC`, {
                    headers: {
                        Authorization: `Bearer ${userToken}`
                    }
                });
    ...
    const deleteFavorite = async (postId) => {
        try {
            const response = await axios.delete(`${Url}favorites/delete/${postId}`, {
                headers: {
                    Authorization: `Bearer ${userToken}`,
                },
            });
                        ...
                        <div className=" h-auto">
                            ...
                            <div className="post-media-grid">
                                {post.medias && post.medias.length > 0 ? (
                                    ...
                                    post.postId.medias && post.postId.medias.length > 0 && (
