...
const PostCard = ({ reloadPosts, avatar }) => {
    ...
    const handleUsernameClick = (userId) => {
        ...
        // Chuyển hướng đến trang cá nhân của người dùng với userId
        // Ví dụ: history.push(`/profile/${userId}`);
    };

    return (
        <div>
            {posts && posts.length > 0 && posts.map(post =>
                post.medias.some(media => media.publicUrl.endsWith('.mp4')) && (
                            ...
                            <div className="flex">
                                <Link to={`/friendProfile/${post.userId.id}`} className="cursor-pointer" onClick={() => handleUsernameClick(post.userId.id)}>
                                    <div className="flex justify-start items-center pb-4 pl-4">
                                        ...
                                        <div className="flex flex-col ml-4">
                                            ...
                                            <p className="text-sm text-gray-700 no-underline tracking-normal leading-none">
                                ...
                                <div className="w-full flex justify-end cursor-pointer" onClick={() => handleSavePost(post.id)}>
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                        ...
                                        className="w-6 h-6 mr-4 hover:text-blue-500"> {/* Thay đổi hover:text-red-500 thành hover:text-blue-500 */}
                                ...
                                <div className="post-media-grid">
                                    {post.medias.map((media, index) => (
                                        media.publicUrl.endsWith('.mp4') && (
                                            <div key={index} className="post-media-item video">
                                                <video className="w-full h-auto" muted controls>
                                                    <source
